...
float32_t DMAMEM interpolation_R_state [num_taps / 2 + N_BLOCKS * BUFFER_SIZE / 2]; 
...
//  setI2SFreq(SAMPLE_RATE);
  ...
  if (arm_fir_interpolate_init_f32(&interpolation_R, (uint8_t)shift, num_taps, interpolation_coeffs, interpolation_R_state, BUFFER_SIZE * N_BLOCKS / (uint32_t)shift)) 
  {
    Serial.println("Init of interpolation failed");
    while(1);
  }
  if (arm_fir_interpolate_init_f32(&interpolation_L, (uint8_t)shift, num_taps , interpolation_coeffs, interpolation_L_state, BUFFER_SIZE * N_BLOCKS / (uint32_t)shift)) 
  {
    Serial.println("Init of interpolation failed");
    while(1);
  }
      ...
      for (unsigned i = 0; i < N_BLOCKS; i++)
        {
          ...
          arm_float_to_q15 (&out_buffer_L[BUFFER_SIZE * i], sp_L, BUFFER_SIZE); 
          arm_float_to_q15 (&out_buffer_R[BUFFER_SIZE * i], sp_R, BUFFER_SIZE);
